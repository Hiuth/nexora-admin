...
export const productImgService = {
  create: async (
    ...
    return apiCall(`${API_CONFIG.ENDPOINTS.PRODUCT_IMG.CREATE}/${productId}`, {
      method: "POST",
      headers: {},
      body: formData,
    });
  ...
  getByProductId: async (
    ...
    apiCall(
      `${API_CONFIG.ENDPOINTS.PRODUCT_IMG.GET_BY_PRODUCT_ID}/${productId}`
    ),

  delete: async (id: string): Promise<ApiResponse<string>> =>
    apiCall(`${API_CONFIG.ENDPOINTS.PRODUCT_IMG.DELETE}/${id}`, {
      method: "DELETE",
    }),
};
